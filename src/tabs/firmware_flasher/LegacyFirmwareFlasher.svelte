<script>
  // This is the Firmware Flasher exactly as it stood before the guided
  // step-wizard rework (and before auto backup/restore existed at all) --
  // kept around, reachable from the new wizard's Connect step, for anyone
  // who wants the older single-page flow back. Deliberately not touched
  // beyond that link back and the onBackToWizard prop below; fix bugs here
  // only if they'd have affected this exact snapshot, not new ones
  // introduced since.
...
<style lang="scss">
  ...
  .legacy-back-link {
    ...
    font-size: 0.8rem;
    cursor: pointer;
    // Header is a flex row (see Page.svelte) - this keeps the link from
    // stretching to share the h1's bold/letter-spaced styling.
    margin-left: auto;
  }
</style>
