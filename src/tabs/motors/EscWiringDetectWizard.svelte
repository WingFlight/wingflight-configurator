<script>
  ...
  // RxWiringDetectWizard.svelte (see wingflight-firmware's
  ...
  // genuinely differs (no `inverted` field, 4 combos not 8), matching this
  // codebase's existing convention of one wizard per feature (AutoAlignWizard
  // vs MountTrimAutoWizard) rather than forcing shared components across
  // unrelated tabs.
  let { onDetected, onButtonDisabled, onClose, onSaveRequested } = $props();

  // Keep in sync with wingflight-firmware's sensors/esc_sensor.h
  ...
  function sendTrialQuery(action) {
    ...
    return new Promise((resolve) => {
      ...
      MSP.send_message(
        MSPCodes.MSP2_WING_ESC_SENSOR_TRIAL,
        payload,
        false,
        finish,
        true,
