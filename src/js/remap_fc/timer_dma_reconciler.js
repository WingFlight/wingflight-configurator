 ...
 * Ported from Rotorflight Configurator PR #433; unchanged apart from
 * collectClashes (a structured form of detectClashes, so the wiring
 * session can attribute each clash to pins) which detectClashes now
 * derives its reasons from.
 *
 * The entry point the apply flow calls before sending its
 ...
 * @property {"timer"|"dma"|"unassigned"} kind
 * @property {string[]} features - Option keys involved (one for a
 *   reserved/unassigned clash, two for a pairwise one).
 * @property {string[]} pins - The pins those features sit on.
 * @property {string} detail - Human-readable description.
 */

/**
 * Structured clash detection over the working set's current timer/DMA
 * state: a feature with no timer chosen yet (every newly assigned/
 * moved one), two features sharing a full timer+channel or a base
 * across feature types, a feature sitting on a timer+channel or base
 * something outside this tool's control has claimed, two features
 * sharing a DMA stream, or a feature sitting on a reserved stream.
 * @param {FeatureTimerRow[]} featureRows
 * @param {Set<string>} [reservedStreams]
 * @param {Set<string>} [reservedTimers]
 * @returns {Clash[]}
 */
export function collectClashes(
  featureRows,
  ...
  for (const row of featureRows) {
    if (row.options.length > 0 && !row.currentOption) {
      clashes.push({
        ...
        pins: [row.pin],
        detail: `${row.feature} (${row.pin}) has no timer chosen yet`,
      });
    }
  }
    ...
    if (row.currentOption) {
      ...
      if (reservedTimers.has(timer)) {
        clashes.push({
          ...
          features: [row.feature],
          pins: [row.pin],
          detail: `${row.feature} sits on ${timer}, which is reserved for another peripheral`,
        });
      }
      ...
      if (timerOwner) {
        clashes.push({
          kind: "timer",
          features: [row.feature, timerOwner.feature],
          pins: [row.pin, timerOwner.pin],
          detail: `${row.feature} and ${timerOwner.feature} both use ${timer}`,
        });
      } else {
        seenFullTimers.set(timer, row);
      }

      const baseOwnerRow = baseOwner.get(base);
      if (baseOwnerRow && baseOwnerRow.type !== row.type) {
        clashes.push({
          kind: "timer",
          features: [row.feature, baseOwnerRow.feature],
          pins: [row.pin, baseOwnerRow.pin],
          detail: `${row.feature} (${row.type}) and ${baseOwnerRow.feature} (${baseOwnerRow.type}) both use base ${base}`,
        });
      } else if (!baseOwnerRow) {
        baseOwner.set(base, row);
      }
    }
    ...
    if (row.currentDma) {
      ...
      if (reservedStreams.has(stream)) {
        clashes.push({
          ...
          features: [row.feature],
          pins: [row.pin],
          detail: `${row.feature} sits on ${stream}, which is reserved for another peripheral`,
        });
      }
      ...
      if (dmaOwner) {
        clashes.push({
          kind: "dma",
          features: [row.feature, dmaOwner.feature],
          pins: [row.pin, dmaOwner.pin],
          detail: `${row.feature} and ${dmaOwner.feature} both use ${stream}`,
        });
      } else {
        seenDma.set(stream, row);
      }
    }
  }
 ...
 * @property {string[]} reasons - Human-readable descriptions, for
 *   logging/debugging -- not shown in the UI.
 * @property {Clash[]} clashes - The structured form (see collectClashes).
 */

/**
 * Checks the working set's current timer/DMA state for anything that
 * needs a reallocation pass -- see collectClashes for the rules.
 * @param {FeatureTimerRow[]} featureRows
...
export function detectClashes(
  ...
  const clashes = collectClashes(featureRows, reservedStreams, reservedTimers);
  const reasons = clashes.map((clash) => clash.detail);
  return { hasClash: reasons.length > 0, reasons, clashes };
}
 ...
 *   wiring_session.svelte.js's apply() writing a just-sent allocation back
